...
class InferenceResultsTab(QWidget):
    ...
    def load_model(self):
        ...
        # If default not set, use home directory
    ...
    def _do_load_model(self, filepath: str, num_classes: int = 2):
        ...
        try:
            ...
            # Try to load metadata to get architecture and input shape details.
            import json as _json
            ...
            if os.path.exists(meta_path):
                try:
                    ...
                    print(f"[InferenceTab] Warning: could not load metadata: {e}")
            ...
            self.model_label.setText(
                f"Loaded: {os.path.basename(filepath)} ({model_name}) on {device_name}"
            )
            self.load_data_btn.setEnabled(True)
        except Exception as e:
    ...
    def load_test_data(self):
        ...
        # Get default data directory from settings
        settings = QSettings("MyCompany", "MixedSignalGUI")
        default_dir = settings.value("dataPath", "")
        
        # If default not set, use current directory
        if not default_dir or not os.path.isdir(default_dir):
            default_dir = ""
        
        folder = QFileDialog.getExistingDirectory(self, "Select Test Data Folder", default_dir)
        if not folder:
            return
    ...
    def evaluate_confusion_matrix(self):
        ...
        try:
            with torch.no_grad():
                data = self.eval_data.to(self.get_device())
